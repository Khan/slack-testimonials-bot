...
secrets.py secrets_decrypt decrypt_secrets:
	@if [ -s "secrets.py" ]; then \
		sed -n 's/secrets_secret = "\(.*\)"/\1/p' secrets.py | openssl cast5-cbc -d -in secrets.py.cast5 -out secrets.py -pass stdin && \
		...
		openssl cast5-cbc -d -in secrets.py.cast5 -out secrets.py; \
	fi
	@chmod 600 secrets.py

# for updating secrets.py
secrets.py.cast5 secrets_encrypt encrypt_secrets:
	sed -n 's/secrets_secret = "\(.*\)"/\1/p' secrets.py | openssl cast5-cbc -e -in secrets.py -out secrets.py.cast5 -pass stdin && \
